...
session_start();
   ...
   if(isset($_POST['curtok']))
   {
      $curtok = $_POST['curtok'];
      $query="UPDATE updatetoken SET tokenno = '$curtok' WHERE sid = '$login_shopid'";
      $result= mysqli_query($con,$query);
      if($result)
      {
        echo "<h1 class='title-font font-medium text-3xl text-black'>Updated successfully</h1>";
      }
   }
?>
<html>
   ...
   <head>
    ...
    <title>Establishment | Update Token Status</title>
   ...
   <body>
      ...
      <section class="text-gray-500 bg-gray-900 body-font">
        <div class="container px-5 py-24 mx-auto flex flex-wrap items-center">
          <div class="lg:w-3/5 md:w-1/2 md:pr-16 lg:pr-0 pr-0">
            <h1 class="title-font font-medium text-3xl text-white">UPDATING STATUS</h1>
            <p class="leading-relaxed mt-4">This section is for shop owners.<br>
            Update the status of current token.</p>
          </div>
      ...
      <form method = "POST" action="updatestatus.php">
       <div class="flex lg:w-2/3 w-full sm:flex-row flex-col mx-auto px-8 sm:px-0">
            <input class="flex-grow w-full bg-gray-800 rounded border border-gray-700 text-white focus:outline-none focus:border-teal-500 text-base px-4 py-2 mr-4 mb-4 sm:mb-0" placeholder="Current Token" type="text" name="curtok">
            <button class="text-white bg-teal-500 border-0 py-2 px-8 focus:outline-none hover:bg-teal-600 rounded text-lg">UPDATE</button>
          </div>
        </form>
