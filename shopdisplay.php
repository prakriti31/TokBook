      ...
      <section class="text-gray-500 bg-gray-900 body-font">
        <div class="container px-5 py-24 mx-auto flex flex-wrap items-center">
          <div class="lg:w-3/5 md:w-1/2 md:pr-16 lg:pr-0 pr-0">
            <h1 class="title-font font-medium text-3xl text-white">City CATALOGUE</h1>
            <p class="leading-relaxed mt-4">This section is for customer.<br>
          </div>
      <center><h1 class="title-font font-medium text-3xl text-white">Welcome <?php echo $login_session;?><br>(CUSTOMER)<br>Select the shop of your choice:<br></h1></center>
      <hr>
      <center>
        <form method = "POST" action="shopdisplay.php">
            <table width=100% border="all" style="background-color: gray; margin-top: 70px;" >
                <tr>
                    ...
                    <th class="thead">Short Description</th>
                    <th class="thead">Book</th>
                    <th class="thead">Check</th>

                </tr>
       <?php 
                    $i=1;
                    $query="SELECT * FROM shoplocation WHERE city = '$login_city'";
                    $result = mysqli_query($con,$query);
                    while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){

                        $sid=$row['sid'];
                        $query1="INSERT into classify (sid,link,cid) values ('$sid','$i','$login_custid')";
                        ...
                        echo   "<tr>
                                    ...
                                    <td>$address</td>
                                    <td>$des</td>
                                    <td><input type='submit' class='text-white bg-teal-500 border-0 py-2 px-8 focus:outline-none hover:bg-teal-600 rounded text-lg' name='book' value=$i></td>
                                    <td><input type='submit' class='text-white bg-teal-500 border-0 py-2 px-8 focus:outline-none hover:bg-teal-600 rounded text-lg' name='check' value=$i></td>
                                </tr>";
                         ...
                         if(isset($_POST['book']))
                    {
                      $id = $_POST['book'];
                      //echo $id;
                      $query = mysqli_query($con,"SELECT sid FROM classify WHERE link = '$id'");
                      $row = mysqli_fetch_array($query, MYSQLI_ASSOC);
                      $shopid = $row['sid'];
                      $_SESSION['upshop'] = $shopid;
                      $sql = "INSERT INTO book(cid,cname,sid,status,reqid) values ('$login_custid','$login_session','$shopid','pending','$id')";
                     ...
                     if(isset($_POST['check']))
                    {
                      ...
                      $query = mysqli_query($con,"SELECT sid FROM classify WHERE link = '$i'");
                      $row = mysqli_fetch_array($query, MYSQLI_ASSOC);
                      $shopid = $row['sid'];
                      $_SESSION['upshop'] = $shopid;
