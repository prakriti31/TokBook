<?php
   include("config.php");
     ...
     session_start();
      ...
      {
         $sql = "INSERT INTO shopdetails(sid,sname,oname,email) values ('$susername','$sname','$oname','$semail')";
         $sql1 = "INSERT INTO shop(sid,password) values ('$susername','$spassword')";
         $result = mysqli_query($con,$sql);
         $result1 = mysqli_query($con,$sql1);
         if($result && $result1)
         {
            $_SESSION['login_shopid'] = $susername;
   ...
   if(isset($_POST['login']))
   {
    $myusername = mysqli_real_escape_string($con,$_POST['username']);
      ...
      $sql = "SELECT * FROM shop WHERE sid = '$myusername' and password = '$mypassword'";
      $result = mysqli_query($con,$sql);
      ...
      if($count == 1) {
         $_SESSION['login_shopid'] = $myusername;
...
<head>
    ...
    <title>Establishment | Login & Register</title>
...
<body >
    <header class="text-gray-500 bg-gray-900 body-font">
      ...
      <hr>
      <form method="post" action="establishmentlogin.php">
      <section class="text-gray-500 bg-gray-900 body-font">
        <div class="container px-5 py-24 mx-auto flex flex-wrap items-center">
          <div class="lg:w-3/5 md:w-1/2 md:pr-16 lg:pr-0 pr-0">
            <h1 class="title-font font-medium text-3xl text-white">ESTABLISHMENT LOGIN & REGISTER</h1>
          ...
          <div class="lg:w-2/6 md:w-1/2 bg-gray-800 rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0">
            ...
            <input class="bg-gray-900 rounded border text-white border-gray-900 focus:outline-none focus:border-teal-500 text-base px-4 py-2 mb-4" placeholder="Establishment's Name" type="text" name="sname">
            <input class="bg-gray-900 rounded border text-white border-gray-900 focus:outline-none focus:border-teal-500 text-base px-4 py-2 mb-4" placeholder="Owner's Name" type="text" name="oname">
            ...
            <button class="text-white bg-teal-500 border-0 py-2 px-8 focus:outline-none hover:bg-teal-600 rounded text-lg" name="upload" id="upload">UPLOAD DOCUMENTS</button>
            <a style="text-align: center;" class="text-xs text-gray-600 mt-3" href="">Forgot password?</a>
            <p class="text-xs text-gray-600 mt-3">Already registered?Login below to start your day with ToKBooK</p>
      ...
      <section class="text-gray-500 bg-gray-900 body-font">
        <div class="container px-5 py-24 mx-auto">
          ...
          <div class="flex lg:w-2/3 w-full sm:flex-row flex-col mx-auto px-8 sm:px-0">
            ...
            <button class="text-white bg-teal-500 border-0 py-2 px-8 focus:outline-none hover:bg-teal-600 rounded text-lg" name="login" id="login">LOGIN</button>
          </div>
        </div>
      </section>
   </form>
