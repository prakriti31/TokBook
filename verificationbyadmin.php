...
<html">
   ...
   <head>
    ...
    <title>Admin | Shop Verification</title>
   ...
   <body>
      ...
      <section class="text-gray-500 bg-gray-900 body-font">
        <div class="container px-5 py-24 mx-auto flex flex-wrap items-center">
          <div class="lg:w-3/5 md:w-1/2 md:pr-16 lg:pr-0 pr-0">
            <h1 class="title-font font-medium text-3xl text-white">VERIFICATION OF SHOP</h1>
      ...
      <center><h1 class="title-font font-medium text-3xl text-white">Sucessfully logged in as <?php echo $login_session; ?><br>(ADMIN)</h1>
      <br><br>
      <form method="POST" action="verificationbyadmin.php">
      <table width=100% border="all" style="background-color: gray; margin-top: 70px;" >
                <tr>
                    <th class="thead">S.No.</th>
                    <th class="thead">Shop</th>
                    <th class="thead">Id proof</th>
                    <th class="thead">Shop Proof</th>
                    <th class="thead">Status</th>
                </tr>
       <?php 
                    $i=1;
                    $query="SELECT * FROM shopdetails WHERE status = 'pending'";
                    $result = mysqli_query($con,$query);
                    while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){

                        $name = $row['sname'];
                        $idproof = $row['idproof'];
                        $shopproof = $row['shopproof'];
                        echo   "<tr>
                                    <td>$i</td>
                                    <td>$name</td>
                                    <td>$idproof</td>
                                    <td>$shopproof</td>
                                    <td><button class='text-white bg-teal-500 border-0 py-2 px-8 focus:outline-none hover:bg-teal-600 rounded text-lg' name='approve'>Approve</button></td>
                                </tr>";
                        
                    if(isset($_POST['approve']))
                    {
                      $sql = "UPDATE shopdetails SET status = 'done' WHERE sno = $i";
